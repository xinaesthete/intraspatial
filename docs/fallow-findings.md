...
| --- | --- | --- | --- |
| `src/gpu/sim/jet.ts` : `Jet3`, `Body`, `neutralBody`, `SwarmBuffers`, `makeSwarmBuffers`, `readBody`, `writeBody`, `swarmFinite` | module with no consumers | **delete** (done) | `--trace` on every export: exactly one reference each, the `export *` in `src/gpu/sim/index.ts`. Grep for `jet` (case-insensitive) finds no importer, no test, no prose — only a comment in `quat.ts`, updated. |
| `src/gpu/graph/ops/danceForces.ts` : `constrainOp`, `swimOp`, `vortexOp`, `solenoidOp`, `orbitOp`, `cohereOp`, `separateOp`, `springOp`, `partnerOrbitOp`, `callerOp`, `clockOp`, `bodyTapOp`, `integrateOp` | 13 per-op exports duplicating `FORCE_OPS` | **refactor** — made module-private (done) | Grep finds no importer of any name outside the file. All 13 remain reachable: they sit in the exported `FORCE_OPS` list (the playground palette) and are registered by `registerForceOps()`. `danceForces.run.test.ts` and `playground/src/extraOps.ts` go through those; `pnpm typecheck` (core + playground) passes. |
